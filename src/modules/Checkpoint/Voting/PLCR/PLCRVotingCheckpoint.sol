...
contract PLCRVotingCheckpoint is PLCRVotingCheckpointStorage, VotingCheckpoint {
    ...
    function revealVote(uint256 _ballotId, uint256 _choiceOfProposal, uint256 _salt) external {
        ...
        require(
            keccak256(abi.encodePacked(_choiceOfProposal, _salt)) == ballot.investorToProposal[msg.sender].secretVote,
            "Invalid vote"
        );
        // Get the balance of the voter (i.e `msg.sender`) at the checkpoint on which ballot was created.
        uint256 weight = securityToken.balanceOfAt(msg.sender, ballot.checkpointId);
        ...
        ballot.proposalToVotes[_choiceOfProposal] = ballot.proposalToVotes[_choiceOfProposal] + weight;
    ...
    function changeBallotStatus(uint256 _ballotId, bool _isActive) external withPerm(ADMIN) {
        ...
        _checkIndexOutOfBound(_ballotId);
        require(
            block.timestamp <= uint256(ballots[_ballotId].startTime)
            + uint256(ballots[_ballotId].commitDuration)
            + uint256(ballots[_ballotId].revealDuration),
            "Already ended"
        );
    ...
    function getCurrentBallotStage(uint256 _ballotId) public view returns (Stage) {
        ...
        uint256 commitTimeEnd = uint256(ballot.startTime) + uint256(ballot.commitDuration);
        uint256 revealTimeEnd = commitTimeEnd + uint256(ballot.revealDuration);

        if (block.timestamp < ballot.startTime)
            return Stage.PREP;
        else if (block.timestamp >= ballot.startTime && block.timestamp <= commitTimeEnd) 
            return Stage.COMMIT;
        else if (block.timestamp > commitTimeEnd && block.timestamp <= revealTimeEnd)
            return Stage.REVEAL;
        else if (block.timestamp > revealTimeEnd)
            return Stage.RESOLVED;
        
        // This should never be reached but is needed to satisfy the compiler
        revert("Invalid ballot stage");
    }

    /**
     * @notice Queries the result of a given ballot
     * @param _ballotId Id of the target ballot
     * @return voteWeighting Array of vote weights for each proposal
     * @return tieWith Array of proposals tied with the winning proposal
     * @return winningProposal ID of the winning proposal
     * @return isVotingSucceed Whether voting succeeded
     * @return totalVotes Total number of voters
     */
    function getBallotResults(uint256 _ballotId) external view returns(
        ...
        uint256 quorumWeight = (supplyAtCheckpoint * ballot.quorum) / 10 ** 18;
    ...
    function getBallotDetails(uint256 _ballotId) external view returns(uint256, uint256, uint256, uint256, uint256, uint256, uint256, bool) {
        ...
        return (
            ...
            ballot.checkpointId,
            ballot.startTime,
            (uint256(ballot.startTime) + uint256(ballot.commitDuration)) + uint256(ballot.revealDuration),

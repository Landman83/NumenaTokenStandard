...
contract GeneralPermissionManager is GeneralPermissionManagerStorage, IPermissionManager, Module {
     ...
     */
    function checkPermission(address _delegate, address _module, bytes32 _perm) external view override returns(bool) {
     ...
     */
    function addDelegate(address _delegate, bytes32 _details) external override withPerm(ADMIN) {
     ...
     * @param _delegate Ethereum address of the delegate
     */
    function deleteDelegate(address _delegate) external override withPerm(ADMIN) {
     ...
     * @return bool Whether the address is a delegate
     */
    function checkDelegate(address _potentialDelegate) external view override returns(bool) {
     ...
     */
    function changePermission(address _delegate, address _module, bytes32 _perm, bool _valid) public override withPerm(ADMIN) {
        require(_delegate != address(0), "invalid address");
     ...
     * @param _module Ethereum contract address of the module
     * @param _perm Permission flag
     * @return address[] Array of delegate addresses
     */
    function getAllDelegatesWithPerm(address _module, bytes32 _perm) external view override returns(address[] memory) {
        uint256 counter = 0;
        uint256 i = 0;
     ...
     */
    function getAllModulesAndPermsFromTypes(address _delegate, uint8[] calldata _types) external view override returns(
        address[] memory,
        bytes32[] memory
    ) {
        uint256 counter = 0;
     ...
     * @return address[] Array of all delegate addresses
     */
    function getAllDelegates() external view override returns(address[] memory) {
        return allDelegates;
    }
     ...
     */
    function checkPerm(address _module, bytes32 _perm) external view override returns(bool) {
        return perms[_module][msg.sender][_perm];
    }
}

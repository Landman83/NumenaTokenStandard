 ...
 */
contract SecurityToken is ReentrancyGuard, SecurityTokenStorage, IERC1594, IERC1643, IERC1644, IERC1410, Proxy {
    // ERC20 storage
    ...
    function _onlyModule(uint8 _type) internal view {
        require(isModule(msg.sender, _type), "Not correct module type");
    }
    ...
    */
     ...
     */
    function transferWithData(address _to, uint256 _value, bytes calldata _data) public {
    ...
    function _balanceOfByPartition(bytes32 _partition, address _tokenHolder, uint256 _additionalBalance) internal view returns(uint256 partitionBalance) {
        address[] storage tms = modules[TRANSFER_KEY];
        uint256 amount;
     ...
     */
    function transferByPartition(bytes32 _partition, address _to, uint256 _value, bytes calldata _data) public returns (bytes32) {
        ...
        returns (bytes32)
    {
        ...
        require(_operatorData.length > 0 && _operatorData[0] != 0, "Invalid operator data");
     ...
     * @return verified Whether the transfer is verified
     */
    ...
    function issueMulti(address[] memory _tokenHolders, uint256[] memory _values) public {
        ...
        require(_tokenHolders.length == _values.length, "Arrays length mismatch");
    ...
    function redeemFrom(address _tokenHolder, uint256 _value, bytes calldata _data) external {
        ...
        // In OpenZeppelin 0.8.x, we need to use _spendAllowance and _burn
    ...
    function setController(address _controller) external {
        _onlyOwner();
        require(isControllable(), "Controller disabled");
    ...
    function canTransferFrom(address _from, address _to, uint256 _value, bytes calldata _data) external view returns (bytes1 statusCode, bytes32 reasonCode) {
        (statusCode, reasonCode) = _canTransfer(_from, _to, _value, _data);
        if (_isSuccess(statusCode) && _value > allowance(_from, msg.sender)) {
        ...
        returns (bytes1 statusCode, bytes32 reasonCode, bytes32 partition)
    {
        if (_partition == UNLOCKED) {
            ...
            if (_isSuccess(statusCode)) {
                ...
                partition = _returnPartition(beforeBalance, afterbalance, _value);
            }
